...
public class VesselTwoDayDelayPercentilesTaskTest {
    @InjectMocks
    VesselTwoDayDelayPercentilesTask vesselTwoDayDelayPercentilesTask;
    ...
    void shouldGetTwoDayDelayPortCallPercentiles() {
        ...
        DataInitializer.initializeVesselPortCallsWithLogEntries(vessels, delaysForEachPort, 2);

        VesselDelayPercentiles vesselDelayPercentiles = vesselTwoDayDelayPercentilesTask.find(vessels);
    ...
    void shouldGetSameTwoDayDelayPortCallPercentilesForSameDelays() {
        ...
        DataInitializer.initializeVesselPortCallsWithLogEntries(asList(vessel), delaysForEachPort, 2);

        VesselDelayPercentiles vesselDelayPercentiles = vesselTwoDayDelayPercentilesTask.find(Collections.singletonList(vessel));
        for (int i = 0; i < vesselDelayPercentiles.getVesselsWithPercentiles().size(); i++) {
            VesselWithPercentiles vesselWithPercentiles =  vesselDelayPercentiles.getVesselsWithPercentiles().get(i);
    ...
    void shouldGetTwoDayDelayPortCallPercentilesForZeroDelays() {
        ...
        DataInitializer.initializeVesselsWithPortCalls(Collections.singletonList(vessel), ports, nrOfPortCallsForEachPort,
                ZonedDateTime.now(), ZonedDateTime.now().plusHours(1));
        DataInitializer.initializeVesselPortCallsWithLogEntries(Collections.singletonList(vessel), delaysForEachPort, 2);
    ...
    void shouldNotGetDelayPercentilesForLogEntriesWithDeparturesOnly() {
        ...
        VesselDelayPercentiles percentiles = vesselTwoDayDelayPercentilesTask.find(Collections.singletonList(vessel));
    ...
    void shouldNotGetDelayPercentilesForNoLogEntries() {
        ...
        VesselDelayPercentiles percentiles = vesselTwoDayDelayPercentilesTask.find(vessels);
        assertEquals(0, percentiles.getVesselsWithPercentiles().size());
    }
}
